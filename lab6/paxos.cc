         ...
         std::string &v)
{
  ...
  for(; i < len; i++){
    ...
    rpcc *cl = h.safebind();
    if(cl) ret = cl->call(paxos_protocol::preparereq, me, a, r, rpcc::to(1000));
        ...
        std::vector<std::string> nodes, std::string v)
{
  ...
  for(; i < len; i++){
    ...
    VERIFY(pthread_mutex_unlock(&pxs_mutex) == 0);
    rpcc *cl = h.safebind();
    if(cl) ret = cl->call(paxos_protocol::acceptreq, me, a, r, rpcc::to(1000));
    VERIFY(pthread_mutex_lock(&pxs_mutex) == 0);
    if(ret == paxos_protocol::OK){
      accepts.push_back(nodes[i]);
    }
	      ...
	      std::string v)
{
  ...
  for(; i < len; i++){
    ...
    VERIFY(pthread_mutex_unlock(&pxs_mutex) == 0);
    rpcc *cl = h.safebind();
    if(cl) ret = cl->call(paxos_protocol::decidereq, me, a, r, rpcc::to(1000));
    VERIFY(pthread_mutex_lock(&pxs_mutex) == 0);
  }
}
